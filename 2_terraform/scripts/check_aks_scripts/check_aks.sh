...
etc_list() {
    # 1. AKS 노드 상태 확인
    run_check "[ETC-1] AKS 노드 상태 확인" \
        "kubectl get nodes -o wide"

    # 2. Ingress 컨트롤러 상태 확인
    run_check "[ETC-2] Ingress 컨트롤러 상태 확인" \
        "kubectl get ingress --all-namespaces"

    # 3. Cluster Autoscaler 상태 확인
    run_check "[ETC-3] Cluster Autoscaler 상태 확인" \
        "az aks show --resource-group $RESOURCE_GROUP --name $AKS_CLUSTER --query \"agentPoolProfiles[].{Name:name, AutoscalerEnabled:enableAutoScaling, Count:count, MinCount:minCount, MaxCount:maxCount}\" --output table"

    # 4. StorageClass 상태 확인
    run_check "[ETC-4] StorageClass(SC) 상태 확인" \
        "kubectl get sc"

    # 5. PV 상태 확인
    run_check "[ETC-5] PersistentVolume(PV) 상태 확인" \
        "kubectl get pv --all-namespaces"

    # 6. Web Application Routing(nginx ingress) 상태 확인
    run_check "[ETC-6] Web Application Routing(nginx ingress) 상태 확인" \
        "az aks show --resource-group $RESOURCE_GROUP --name $AKS_CLUSTER --query \"ingressProfile.webAppRouting.enabled\" --output json"
}
