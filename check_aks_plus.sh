...
export API_SERVER_LOGS_STATUS=$?
if [ $API_SERVER_LOGS_STATUS -eq 0 ]; then
    log "✅ API 서버 로그 조회 성공 (최근 10개):"
    # API 서버 로그를 한 줄씩 로깅
    echo "$API_SERVER_LOGS" | while IFS= read -r line; do
        log "   $line"
    done
else
    log "❌ API 서버 로그 조회 실패 (오류 코드: $API_SERVER_LOGS_STATUS). 대체 방법: AKS Diagnostics 사용"
    log "   👉 az aks diagnose --resource-group $RESOURCE_GROUP --name $AKS_CLUSTER"
fi

# 🔹 5. 어플리케이션 Pod 로그 확인 (모든 Pod, 각각 최근 10개 로그)
log "🔍 어플리케이션 Pod 로그 조회 중..."
export POD_NAMES=$(kubectl get pods -n "$NAMESPACE" --no-headers -o custom-columns=":metadata.name" 2>/dev/null)
export POD_LIST_STATUS=$?

if [ $POD_LIST_STATUS -ne 0 ]; then
    log "❌ Pod 목록 조회 실패 (오류 코드: $POD_LIST_STATUS). 네임스페이스($NAMESPACE)를 확인하세요."
elif [ -z "$POD_NAMES" ]; then
    log "⚠️ Pod를 찾을 수 없습니다. 네임스페이스($NAMESPACE)를 확인하세요."
else
    log "✅ 네임스페이스 $NAMESPACE에서 Pod 목록 조회 성공"
    # 모든 Pod에 대해 로그 조회
    echo "$POD_NAMES" | while read -r pod_name; do
        if [ -n "$pod_name" ]; then
            log "🔍 Pod ($pod_name) 로그 조회 중..."
            export POD_LOGS=$(kubectl logs "$pod_name" -n "$NAMESPACE" --tail=10 2>&1)
            export POD_LOGS_STATUS=$?
            
            if [ $POD_LOGS_STATUS -eq 0 ]; then
                log "✅ Pod ($pod_name) 로그 조회 성공 (최근 10개):"
                # Pod 로그를 한 줄씩 로깅
                echo "$POD_LOGS" | while IFS= read -r line; do
                    log "   $line"
                done
            else
                log "❌ Pod ($pod_name) 로그 조회 실패 (오류 코드: $POD_LOGS_STATUS)"
                
                # 컨테이너가 여러 개인 경우 각 컨테이너의 로그 조회 시도
                export CONTAINERS=$(kubectl get pod "$pod_name" -n "$NAMESPACE" -o jsonpath='{.spec.containers[*].name}' 2>/dev/null)
                if [ -n "$CONTAINERS" ]; then
                    for container in $CONTAINERS; do
                        log "🔍 Pod ($pod_name), 컨테이너 ($container) 로그 조회 중..."
                        export CONTAINER_LOGS=$(kubectl logs "$pod_name" -c "$container" -n "$NAMESPACE" --tail=10 2>&1)
                        export CONTAINER_LOGS_STATUS=$?
                        
                        if [ $CONTAINER_LOGS_STATUS -eq 0 ]; then
                            log "✅ Pod ($pod_name), 컨테이너 ($container) 로그 조회 성공 (최근 10개):"
                            echo "$CONTAINER_LOGS" | while IFS= read -r line; do
                                log "   $line"
                            done
                        else
                            log "❌ Pod ($pod_name), 컨테이너 ($container) 로그 조회 실패 (오류 코드: $CONTAINER_LOGS_STATUS)"
                        fi
                    done
                fi
            fi
            ...
            # Pod 상태 정보 추가
            export POD_STATUS=$(kubectl get pod "$pod_name" -n "$NAMESPACE" -o jsonpath='{.status.phase}' 2>/dev/null)
            export POD_READY=$(kubectl get pod "$pod_name" -n "$NAMESPACE" -o jsonpath='{.status.containerStatuses[0].ready}' 2>/dev/null)
            export POD_RESTARTS=$(kubectl get pod "$pod_name" -n "$NAMESPACE" -o jsonpath='{.status.containerStatuses[0].restartCount}' 2>/dev/null)
            
            log "📊 Pod ($pod_name) 상태 정보:"
            log "   - 상태: $POD_STATUS"
            log "   - Ready: $POD_READY"
            log "   - 재시작 횟수: $POD_RESTARTS"
            log "-----------------------------------"
        fi
    done
fi
